...
export class TaskComponent implements OnInit {
  ...
  finishOrRestartTask(task) {
    if (task.status === 'CREATED') {
      this.taskService.finishTask(task).subscribe(() => {
        this.dataSaved = true;
        this.massage = 'Task Finished';
        this.loadAllTasks();
        this.idTaskUpdate = null;
        this.taskForm.reset();
      });
    } else {
      this.taskService.restartTask(task).subscribe(() => {
        this.dataSaved = true;
        this.massage = 'Task Restarted';
        this.loadAllTasks();
        this.idTaskUpdate = null;
        this.taskForm.reset();
      });
